...
class DDPGAgent:
    ...
    def __init__(self, meta_dim: int = 0, **kwargs: tp.Any) -> None:
        ...
        self.actor = Actor(cfg.obs_type, self.obs_dim, self.action_dim, cfg.feature_dim, cfg.hidden_dim).to(cfg.device)

        self.critic: nn.Module = Critic(
            cfg.obs_type, self.obs_dim, self.action_dim, cfg.feature_dim, cfg.hidden_dim
        ).to(cfg.device)
        self.critic_target: nn.Module = Critic(
            cfg.obs_type, self.obs_dim, self.action_dim, cfg.feature_dim, cfg.hidden_dim
        ).to(cfg.device)
        ...
        if self.reward_free:
            self.reward_model = mlp(
                self.obs_dim, cfg.hidden_dim, "ntanh", cfg.hidden_dim, "relu", cfg.hidden_dim, "relu", 1  # type: ignore
            ).to(
                cfg.device
            )  # type: ignore
